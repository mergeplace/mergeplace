			...
			<main class='events__main animated d05 delay-04s fadeInLeft'>
				<nav class='events__nav'>
					<div class='events__nav-inner'>
						<a
              class="events__button events__button--future"
              id='future-button'
              href="#"
              ...
              @click.prevent='toggleEvent'>{{ $t('events.nav.past') }}
					...
					<div class="events__nav-line">
						<div
              class='events__nav-active'
              :style='[activeButtonWidth, translateStyle]'>
            </div>
            <div
              class='events__nav-active events__nav-active--mobile'
              :class='{"events__nav-active--translate": !toggleButton}'>
            </div>
					</div>
				</nav>
				<transition
					...
					mode='out-in'>
				<section
          class='events__section events__section--future'
          v-if='toggleButton'
          key="future">
					<article class='event' v-for="(event, index) in getFutureEvents" :key="event.id">
              ...
              <p
                ...
                @click.prevent="+toggleIndex != +index?toggleIndex = index:toggleIndex = -1">{{ $t('events.buttons.more') }}
						...
						<section class="event__info">
							...
							<p class="event__price">{{ event.price === 'free'? $t('access'): event.price }}</p>
				...
				<section
          class='events__section events__section--past'
          v-else key="past">
					<article class='event' v-for="(event, index) in getPastEvents" :key="event.id">
						<section class="event__content">
							<h2 class="event__title">{{ event.title }}</h2>
							<p class="event__subtitle">{{ event.subtitle }}</p>
							<p
...
export default {
    ...
    setStyles() {
      ...
      if (this.toggleButton) {
        element = 'future-button';
        this.translateStyle.transform = `translateX(0)`;
      } else {
        element = 'past-button';
        this.translateStyle.transform = `translateX(${document.getElementById('future-button').offsetWidth}px)`;
      }
      this.activeButtonWidth.width = `${document.getElementById(element).offsetWidth}px`;
    }
	},
	computed: {
		...mapGetters('events', [
